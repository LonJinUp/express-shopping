...
export async function runCommerceTimeoutScan() {
	...
	try {
		...
		const [closedCount, receivedCount, afterSaleReminderCount, dataRetention] = await Promise.all([
			closeExpiredOrders(),
			autoReceiveOrders(),
			remindOverdueAfterSales(),
			runDataRetentionCleanup(),
		])
		const result = { closedCount, receivedCount, afterSaleReminderCount, paymentReconciliation, dataRetention }
		recordTaskCompleted(taskName, result)
		if (closedCount > 0 || receivedCount > 0 || afterSaleReminderCount > 0 || paymentReconciliation.queriedCount > 0) {
